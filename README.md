...
* Magic methods & helpers to make your life easy: `isThere`, `getAttribute`, `findChildEl`, `findElNearBy`, `chooseRandom`, `findFirstElementOnScreen`, `random`, `chance`, `randInt` etc.
...
### ▶ findElementAnywhere(selector: string, text: string | null = null, timeout: number = 10, startTime: number = Date.now()): Promise<object>

Finds the specified element anywhere on the page or within frames.

### ▶ getFrame(startWith: string = '', debug: boolean = false): Promise<object>

Get the frame that starts with a specified URL.

### ▶ isElementInView(selector: string | object, target: object = this.page): Promise<{ isInView: boolean, direction: string }>
...
### ▶ jitterMouse(options: object): Promise<void>
...
### ▶ waitTillHTMLRendered(minStableSizeIterations: number = 3, timeout: number = 15): Promise<void>
