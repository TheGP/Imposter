...
export default class ImposterClass {
    ...
    async type(selector: iSelector, string: string, keepExistingText = false) {
        ...
        if (!isInputFocused || (isInputFocused && this.chance(this.behavior.typing.noticing_focus))) {
    ...
    async typeFixMistake(el, target, shouldbeValue: string, currentValue: string, attempt: number = 0) {
        ...
        const type = await el.evaluate((element: HTMLInputElement) => element.getAttribute('type'));
        ...
        while (null === cursorPosition) {
            cursorPosition = await target.evaluate((el: HTMLInputElement) => el.selectionStart, el);
            ...
            if (null === cursorPosition) {
                ...
                if (stopInMilliseconds <= Date.now() - startTime) {
                    ...
                    await target.evaluate((el: HTMLInputElement, target) => {
    ...
    async click(selectorOrObj: iSelector, text: null | string = null, timeout: number = 10, attempt: number = 1): Promise<ElementHandle | boolean> {
        ...
        const { el, target, type } : iElement = ('string' === typeof selectorOrObj) 
        ...
        if (!res.isInView) {
            ...
            const closestScrollableDiv = await el.evaluateHandle((element: Element | null) => {
                while (element) {
    ...
    async clickSimple(selectorOrObject: string | ElementHandle, attempt: number = 0) {
    ...
    async typeSimple(selector: string | ElementHandle, string: string): Promise<void> {
    ...
    async scrollTo(selector: string | ElementHandle, target: Page | Frame): Promise<boolean> {
    ...
    async getAttribute(elObjOrSelector: iSelector, attribute_name: string, timeout = 0) {
    ...
    async isThere(selector: string, text: null | Function | string = null, timeout: number | Function = 0, cbTrue: null | Function = null, cbElse: null | Function = null) {
        await this.waitTillHTMLRendered();
    ...
    async block(selector: string | Function | null, text: null | Function = null, timeout: number | Function = 0.1, cb: null | Function = null) {
    ...
    async getChildEl(parentEl: ElementHandle | iElement, selector: string, text: null | string = null): Promise<null | ElementHandle> {
        ...
        }, el, selector, text) as ElementHandle<HTMLElement>;
    ...
    async waitForDissapear(selector, text: string | null = null, ignoreVisibility: boolean = true, timeout: number = 120, startTime: number = Date.now()) {
    ...
    async findElementAnywhere(selector: string, text: string | null = null, timeout: number = 10, ignoreVisibility: boolean = false, noDigging: boolean = false, startTime: number = Date.now()) {
    ...
    async findClosestParentEl(selectorChild: ElementHandle | string, selectorParent: string, childText: string | null = null, timeout = 10) {
        childText = this.translate(childText);
        const { el, target, type } = ('object' === typeof selectorChild) 
                                        ? selectorChild 
        ...
        return {
            el : await target.evaluateHandle((el: HTMLElement, selector: string) => {
    ...
    async findChildEl(elObjOrSelector: iElement, selectorChild: string, textChild: string | null = null) {
    ...
    async chooseRandom(selector: string = '', parent: null | iElement = null, except: Element[] = []): Promise<iElement> {
    ...
    async isElementInView(selector: string | ElementHandle, target: Page | Frame = this.page, attempt = 0): Promise<{ isInView: boolean, direction: Direction} | null> {
        ...
        try {
            ...
            if (attempt < 10) {
                ...
                console.error(e);
                if (e.message.includes('Cannot read properties of undefined (reading \'getBoundingClientRect\')')) {
                    return null;
                } else {
    ...
    */
    async findFirstElementOnScreen(selector: string, attempt: number = 0): Promise<false | ElementHandle> {
    ...
    async waitTillHTMLRendered(minStableSizeIterations = 3, timeout = 60): Promise<void> {
    ...
    async isThereIframe(url: string | RegExp, timeout = 0, startTime = Date.now()): Promise<boolean> {
        await this.waitTillHTMLRendered();
